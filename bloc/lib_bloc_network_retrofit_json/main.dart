...
extension LoadPageExt on PPBloc {
  void _loadPage(int offset, {bool force = true}) {
    // final client = _RestPortfolioPlatform();
    var dio = Dio();
    final client = RestPortfolioPlatform(dio);
    client.getPortfolios().then((profileQueryResult) {
      if (force) {
      ...
      _paginatedList.add(PaginatedList(
          pageSize: PPBloc.LIMIT,
          list: profileQueryResult.list,
          totalCount: profileQueryResult.count));
    });
  }
